...
import { Mail } from "lucide-react"; // E-Mail-Icon
...
export default function TeamInviteGenerator() {
  ...
  // Option 3: Einmal-Link direkt via WhatsApp oder E-Mail
  const generateAndSendLink = async (mode: "whatsapp" | "email") => {
    ...
    if (mode === "whatsapp") {
      // Bei mobilen Browsern funktioniert window.location.href besser als window.open
  ...
  // Wiederverwendbare Card-Komponente für Option 1/2/3
  const Card = ({
    title,
    description,
    button,
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
      ...
      <Card
        ...
        description="Dieser QR-Code ist passwortgeschützt (Passwort siehst du unter /team/security). Das Passwort wechselt automatisch alle 24 Stunden, der QR bleibt aber gültig."
        button={
          <button
      ...
      <Card
        ...
        description="Dieser Link ist nach erstem Klick ungültig. Ideal zum direkten Versand an neue Teammitglieder."
        button={
          <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
            {/* WhatsApp-Button */}
            <button
              onClick={() => generateAndSendLink("whatsapp")}
              className="flex items-center justify-center w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded transition"
              disabled={loadingType === "link-whatsapp"}
              title="Per WhatsApp senden"
            >
              {/* Inline-SVG WhatsApp-Logo (weiß auf grünem Hintergrund) */}
              <svg
            ...
            <button
              ...
              className="flex items-center justify-center w-full sm:w-auto bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded transition"
              disabled={loadingType === "link-email"}
              title="Per E-Mail senden"
            >
              <Mail className="w-5 h-5 mr-2" />
              {loadingType === "link-email"
                ? "Erzeuge Link…"
                : isMobile
                ? "Per E-Mail senden"
                : "Per E-Mail öffnen"}
