...
export default function TeamInviteGenerator() {
  ...
  const [expiresIn, setExpiresIn] = useState(48); // Stunden
  ...
  const handleCreateInvite = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || "Fehler beim Erstellen");
      }
    } catch (err) {
  ...
  return (
    <div className="p-4 border rounded shadow max-w-lg mx-auto bg-white">
      ...
      <label className="block mb-2 font-semibold">Rolle:</label>
      ...
      <label className="block mb-2 font-semibold">Gültigkeit (in Stunden):</label>
      <input
        type="number"
        value={expiresIn}
      ...
      {inviteUrl && (
        <div className="mt-6 text-center">
          <h3 className="font-bold mb-2">Einladungslink:</h3>
