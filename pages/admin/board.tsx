    ...
    <div className="max-w-6xl mx-auto px-4 py-8 relative">
      ...
      <div className="bg-white border border-gray-200 p-4 rounded shadow-sm mb-6">
        ...
        <div className="flex gap-2">
          {editId ? (
            ...
            <button onClick={handleManualAdd} className="bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 rounded">Hinzufügen</button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filteredEntries.map((entry) => {
          ...
          return (
            <div key={entry.id} className={`border p-4 rounded-md shadow-sm hover:shadow transition ${isDone ? "bg-green-50 border-green-300" : "bg-white border-gray-200"}`}>
              ...
              <p className="text-xs text-gray-400 mt-4">
                ...
                {entry.completedAt && (
                  <>
                    <br />Fertig: {new Date(entry.completedAt).toLocaleString()}
                  </>
                )}
              </p>
