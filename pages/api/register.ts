...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!email || !emailValid || !password || !isStrongPassword(password)) {
    return res.status(400).json({ message: 'Passwort zu schwach. Es muss mindestens 8 Zeichen, Groß-/Kleinbuchstaben, Zahl und Sonderzeichen enthalten.' });
  }
  ...
  try {
    ...
    return res.status(500).json({ message: 'Serverfehler bei der Registrierung.', detail: error instanceof Error ? error.message : String(error) });
  }
}
